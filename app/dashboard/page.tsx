...
import { 
  ...
  User, Building2, Phone, Mail, MapPin, Loader2, Eye, EyeOff, AlertTriangle, Check, X, Shield
} from "lucide-react";
...
export default function DashboardPage() {
  ...
  const [loading, setLoading] = useState(true);
  const [userProfiles, setUserProfiles] = useState<Map<string, 'admin' | 'regular'>>(new Map());
  ...
  const loadContacts = useCallback(async () => {
    ...
    if (error) {
      ...
      // Load user profiles to check admin status
      const { data: profiles } = await userService.getUsers();
      if (profiles) {
        const profileMap = new Map<string, 'admin' | 'regular'>();
        profiles.forEach(profile => {
          profileMap.set(profile.email.toLowerCase(), profile.role);
        });
  ...
  const deleteSelectedContacts = async () => {
    ...
    setLoading(true);
    const { deletedIds, errors, error } = await contactService.deleteMultipleContacts(selectedContacts);
    ...
    if (error) {
      ...
      // Only remove contacts that were actually deleted
      setContacts(contacts.filter(c => !deletedIds.includes(c.id)));
      setSelectedContacts([]);
      
      // Show detailed feedback
      if (errors.length > 0) {
        const protectedCount = errors.length;
        const deletedCount = deletedIds.length;
        toast({
          title: `⚠️ Partial Delete Complete`,
          description: `${deletedCount} contacts deleted. ${protectedCount} contacts protected (admin users cannot be deleted).`,
          duration: 6000
        });
      } else {
        toast({
          title: "Bulk Delete Complete",
          description: `${deletedIds.length} contacts have been deleted`
        });
      }
    }
        ...
        <div className="mb-8 bg-gradient-to-r from-cyan-600 via-blue-600 to-indigo-700 rounded-2xl shadow-2xl p-8 text-white">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              ...
              {/* User Info */}
              <div className="mt-4 flex items-center gap-3 ml-15">
                <div className="flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-sm rounded-lg border border-white/20">
                  <User className="w-4 h-4" />
                  <span className="font-medium">{user.email}</span>
                </div>
                {isAdmin && (
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-purple-500 to-pink-500 rounded-lg shadow-lg">
                    ...
                    <div className="space-y-2 max-h-96 overflow-y-auto">
                      {contacts.length === 0 ? (
                        ...
                        contacts.map((contact) => {
                          ...
                          const isAdmin = userProfiles.get(contact.email.toLowerCase()) === 'admin';
                          return (
                          <div 
                            key={contact.id} 
                            className={`group flex items-center gap-4 p-4 border-2 rounded-xl transition-all duration-200 hover:shadow-lg ${
                              isSelected 
                                ...
                                : isAdmin
                                ? 'border-purple-300 bg-gradient-to-r from-purple-50 to-pink-50 hover:border-purple-400 hover:shadow-md'
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                ...
                                {isAdmin && (
                                  <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-gradient-to-r from-purple-600 to-pink-600 text-white shadow-sm">
