...
export default function ContactForm({ mode, contact, onSuccess }: ContactFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "add") {
        // Check for duplicates based on extension + email combination
        const { data: existing } = await supabase
          .from("contacts")
          .select("id, name, extension, email")
          .or(`extension.eq.${formData.extension},email.eq.${formData.email}`);

        if (existing && existing.length > 0) {
          const duplicateContact = existing[0];
          toast({
            title: "Duplicate Found",
            description: `Duplicate found for Extension ${duplicateContact.extension}: ${duplicateContact.name}`,
            variant: "destructive",
          });
          return;
        }
