...
export default function SimplePatentableIdeas() {
  ...
  const loadIdeas = useCallback(async () => {
    ...
    setLoading(true);
    const { data, error } = await patentableIdeasService.getIdeasByEmail(user.email);
    if (data) {
      const filtered = selectedUser === "__all__" ? data : data.filter((i: PatentableIdea) => i.email === selectedUser);
      setIdeas(filtered);
    }
    setLoading(false);
  ...
  // Start editing
  const startEdit = (idea: PatentableIdea) => {
    console.log('🔵 Starting edit for idea:', idea.id, idea);
  ...
  const handleUpdate = async (id: string) => {
    ...
    setLoading(true);
    const { data, error } = await patentableIdeasService.updateIdea(id, editForm);
    if (data) {
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-8 text-gray-500">
              No patentable ideas found
            </div>
          ) : (
            <div className="space-y-4 max-h-96 overflow-y-auto">
              {ideas.map(idea => {
                const isEditing = editingId === idea.id;
                console.log('🔴 Rendering idea:', idea.id, 'isEditing:', isEditing, 'editForm:', editForm);
                return (
                  ...
                  {isEditing ? (
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2">
                        <Button size="sm" onClick={() => handleUpdate(idea.id)} disabled={loading}>
                    ...
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex gap-1 ml-4">
                        <Button variant="ghost" size="sm" onClick={() => startEdit(idea)}>
