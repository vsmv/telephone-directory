...
export default function SimpleLearningPlans() {
  ...
  const updatePlan = async (planId: string) => {
    ...
    if (!planToUpdate.title || !planToUpdate.description || !planToUpdate.category) {
      toast({
        title: "Validation Error",
        description: "Title, description, and category are required fields",
        variant: "destructive"
    ...
    const normalizedStatus =
      planToUpdate.status === 'In Progress' ? 'in-progress' :
      planToUpdate.status === 'Completed' ? 'completed' :
      planToUpdate.status === 'On Hold' ? 'archived' :
      (planToUpdate.status === 'not-started' || planToUpdate.status === 'in-progress' || planToUpdate.status === 'completed' || planToUpdate.status === 'archived') ? planToUpdate.status : 'not-started';

    const payload = {
      title: planToUpdate.title,
      description: planToUpdate.description,
      category: planToUpdate.category,
      status: normalizedStatus as 'not-started' | 'in-progress' | 'completed' | 'archived',
      target_completion_date: planToUpdate.target_completion_date || undefined
    };
