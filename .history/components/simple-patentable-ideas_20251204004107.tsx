...
export function SimplePatentableIdeas({ userEmail, userRole = 'regular' }: SimplePatentableIdeasProps) {
  ...
  const handleAddIdea = async () => {
    ...
    try {
      ...
      if (result.data) {
        // Add to both ideas and filteredIdeas to show immediately
        const updatedIdeas = [result.data, ...ideas];
        setIdeas(updatedIdeas);
        
        // Filter to show only user's ideas
        const userIdeas = updatedIdeas.filter(idea => idea.email === userEmail);
        setFilteredIdeas(userIdeas);
