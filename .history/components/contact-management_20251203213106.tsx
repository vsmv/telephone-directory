...
import { Edit, Trash2, Users, Check, X, AlertTriangle } from "lucide-react";
...
export default function ContactManagement() {
  ...
  useEffect(() => {
    ...
    const handleContactsUpdated = () => {
      console.log('📢 Contacts updated event received - reloading...');
      loadContacts();
    };
    ...
    if (error) {
      ...
      if (errors.length > 0) {
        ...
        toast({
          ...
          description: `${deletedIds.length} contacts have been successfully deleted`
        });
      }
      loadContacts();
      setSelectedContacts([]);
      ...
      <Dialog open={!!deleteConfirmContact} onOpenChange={() => setDeleteConfirmContact(null)}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button
              variant="destructive"
              onClick={() => deleteConfirmContact && handleDeleteContact(deleteConfirmContact.id)}
            >
              Delete Contact
            </Button>
          </DialogFooter>
