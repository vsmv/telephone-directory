...
import { Users, User, Building2, BookOpen, Lightbulb, Loader2, Database, Activity, HardDrive, Zap, Server, Clock, Lock, Eye, EyeOff } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Distribution of users by role in the system - Click to view details
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            <div 
              className="flex items-center justify-between p-4 bg-purple-50 rounded-lg cursor-pointer hover:bg-purple-100 transition-colors border border-purple-200"
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                console.log('Navigating to admin users report');
                window.location.href = '/reports?type=users&filterBy=role&filterValue=admin';
              }}
            >
              <span className="font-medium text-purple-900">Admin Users</span>
              <Badge className="bg-purple-100 text-purple-800 border-purple-300 cursor-pointer">
                {dashboardData.adminUsers} ({dashboardData.totalUsers > 0 ? Math.round((dashboardData.adminUsers / dashboardData.totalUsers) * 100) : 0}%)
              </Badge>
            </div>
            <div 
              className="flex items-center justify-between p-4 bg-orange-50 rounded-lg cursor-pointer hover:bg-orange-100 transition-colors border border-orange-200"
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                console.log('Navigating to regular users report');
                window.location.href = '/reports?type=users&filterBy=role&filterValue=regular';
              }}
            >
              <span className="font-medium text-orange-900">Regular Users</span>
