...
export function SimpleLearningPlans({ userEmail, userRole = 'regular' }: SimpleLearningPlansProps) {
  ...
  const handleAddPlan = async () => {
    ...
    try {
      ...
      if (result.data) {
        // Add to both plans and filteredPlans to show immediately
        const updatedPlans = [result.data, ...plans];
        setPlans(updatedPlans);
        
        // Filter to show only user's plans
        const userPlans = updatedPlans.filter(plan => plan.email === userEmail);
        setFilteredPlans(userPlans);
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {filteredPlans.map(plan => (
                <Card key={plan.id} className={`border-l-4 border-l-emerald-500 hover:shadow-md transition-shadow ${selectedPlans.includes(plan.id) ? 'bg-emerald-50' : ''}`}>
      ...
      <Dialog open={bulkDeleteConfirm} onOpenChange={() => setBulkDeleteConfirm(false)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete <strong className="text-gray-900">{selectedPlans.length} learning plans</strong>?
              <br />
              This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
