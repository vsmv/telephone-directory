...
class ContactService {
  ...
  async deleteMultipleContacts(ids: string[]): Promise<{ deletedIds: string[]; errors: any[]; error: string | null }> {
    try {
      ...
      if (!response.ok) {
        return { deletedIds: [], errors: [], error: result.error || 'Failed to delete contacts' };
      }
      
      // Extract IDs of successfully deleted contacts and errors
      const deletedIds = (result.deleted || []).map((c: any) => c.id);
      const errors = result.errors || [];
      
      console.log(`✅ Bulk delete complete: ${deletedIds.length} deleted, ${errors.length} errors`);
      if (errors.length > 0) {
        console.log('⚠️ Deletion errors:', errors);
      }
      
      return { deletedIds, errors, error: null };
    } catch (error: any) {
      console.error('💥 API bulk delete error:', error);
      return { deletedIds: [], errors: [], error: `Failed to delete contacts: ${error.message}` };
    }
  }
  ...
  async bulkInsertContacts(contacts: ContactInsert[]): Promise<{ data: { inserted: Contact[]; skipped: any[] } | null; error: string | null }> {
    try {
      console.log(`📦 BULK INSERT: Processing ${contacts.length} contacts via API`);
      ...
      if (!response.ok) {
        return { data: null, error: result.error || 'Failed to bulk insert contacts' };
      }
      
      console.log(`🎉 BULK INSERT COMPLETE via API: ${result.summary?.inserted || 0} inserted, ${result.summary?.skipped || 0} skipped`);
      return { data: result.data, error: null };
    } catch (error: any) {
      console.error('💥 Bulk insert API error:', error);
      return { data: null, error: `Bulk insert failed: ${error.message}` };
    }
  }
}
...
class UserService {
  ...
  async getUsers(): Promise<{ data: UserProfile[] | null; error: string | null }> {
    try {
      ...
      console.log('User IDs in database:', data?.map(u => ({ id: u.id, email: u.email })));
      return { data, error: null };
    } catch (error: any) {
      console.error('💥 Database user read error:', error);
      return { data: null, error: error.message };
    }
  }
  ...
  async updateUser(id: string, updates: Partial<UserProfileInsert>): Promise<{ data: UserProfile | null; error: string | null }> {
    try {
      console.log(`👤 Updating user ${id} via API with:`, updates);
      ...
      console.log('✅ User updated successfully via API:', result.data);
      return { data: result.data, error: null };
    } catch (error: any) {
