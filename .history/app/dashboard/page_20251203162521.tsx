...
function SettingsTabContent({ user, isAdmin }: { user: any; isAdmin: boolean }) {
  ...
  useEffect(() => {
    const loadStats = async () => {
      ...
      setLoading(true);
      try {
        // Fetch user's own ideas and plans
        const [ideasResult, plansResult, contactsResult] = await Promise.all([
          patentableIdeasService.getIdeasByEmail(user.email),
          learningPlansService.getPlansByEmail(user.email),
          contactService.getContacts()
        ]);

        // If admin, also fetch total users count
        let totalUsers = 0;
        if (isAdmin) {
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Card className="shadow-lg border-0 overflow-hidden">
            ...
            <CardContent className="space-y-3 pt-4">
              ...
              <Button className="w-full bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white shadow-md">Update Password</Button>
            </CardContent>
          </Card>
